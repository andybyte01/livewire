<div x-data="{ focused: false }" class="relative">
    ...
    @elseif($existing)
        ...
        {{--  <img src="{{ Storage::disk('public')->url($article->image) }}" class="border-2 rounded"
                                alt=""> --}}
    ...
    @else
        <div class="h-32 bg-gray-50 border-2 border-dashed rounded flex items-center justify-center">
            <label for="{{ $id }}"
                ...
                :class="{ 'outline-none border-gray-900 ring ring-gray-300': focused }">{{ __('Select Image') }}
            </label>
        </div>
    @endif
    @unless($image)
        <x-jet-input x-on:focus="focused = true" x-on:blur="focused = false" wire:model="{{ $id }}"
            :id="$id" class="sr-only" type="file" />
    @endunless
</div>
